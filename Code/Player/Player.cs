...
public sealed partial class Player : Component, Component.IDamageable, PlayerController.IEvents, Global.ISaveEvents, IKillSource
{
	...
	void OnControl()
	{
		// === BLOCK OLD SANDBOX MENUS ===
		if ( Input.Pressed( "spawnmenu" ) || Input.Pressed( "menu" ) ||
			 Input.Pressed( "context" ) || Input.Pressed( "inspect" ) ||
			 Input.Down( "SpawnMenu" ) || Input.Down( "InspectMenu" ) )
		{
			return;   // Block Q and C menus
		}
		else
		{
