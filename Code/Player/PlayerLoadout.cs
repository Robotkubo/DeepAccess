...
public sealed class PlayerLoadout : Component, Local.IPlayerEvents, Global.IPlayerEvents, Global.ISaveEvents
{
	...
	public struct LoadoutEntry
	{
		...
		// SpawnerDataPayload removed - not needed for horror game
	}
	...
	public string SerializeLoadout()
	{
		var entries = Inventory.Weapons
			.Where( w => w.GameObject.IsValid() && !string.IsNullOrEmpty( w.GameObject.PrefabInstanceSource ) )
	...
	public void GiveLoadoutWeapons( string json )
	{
		...
		try
		{
			...
			{
				Inventory.Pickup( entry.PrefabPath, entry.Slot, false );
			}
		}
		finally
		{
	...
	private async Task RestoreOnSpawnAsync()
	{
		if ( Player.IsLocalPlayer )
		{
			// Clear any previously saved hotbar/loadout so we start fresh
			LocalData.Set<string>( "hotbar", null );
			// Alternative: LocalData.Remove( "hotbar" );  // if this method exists
		}

		await Task.Yield();

		// Give our clean default weapons
		Inventory.GiveDefaultWeapons();
